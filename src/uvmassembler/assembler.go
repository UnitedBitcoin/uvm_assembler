...
func (assembler *Assembler) ParseOperand(operand *Operand, line string, limit int) (bool, int, string) {
	...
	switch line[cpos] {
	case '%': // stack index
		if (limit&LIMIT_STACKIDX) == 0 || cpos >= len(line) {
		...
		return true, lend, ""
	case '@': // upvalue index.
		if (limit&LIMIT_UPVALUE) == 0 || cpos >= len(line) {
	...
	case '$': // jmp location
		if (limit&LIMIT_LOCATION) == 0 || cpos >= len(line) {
	...
	default:
		...
		if (limit & (LIMIT_EMBED | LIMIT_CONSTANT)) == 0 {
...
func (assembler *Assembler) parseCode(line string, lineLen int) (bool, string) {
	...
	if useExtended {
		...
		SET_OPCODE(&extendedInst, OpCode(OP_EXTRAARG))
...
func (assembler *Assembler) ParseAsmContent(asmContent string, outFilepath string) (string, int) {
	...
	if len(assembler.instructions) > 0 {
		res, err := assembler.finalizeFunction()
		if !res {
			panic(err)
		}
	}
	...
	if !assembler.bUpvalues {
		panic("amount of upvalues never declared")
	}

	if ok, err := assembler.writeHeader(); !ok {
		panic(err)
	}
	if BufferWriteInt8(assembler.wBuffer, uint8(assembler.nUpvalues)) != nil {
		panic("failed to write num upvalues")
	}
	...
	ioutil.WriteFile(outFilepath, assembler.wBuffer.Bytes(), 0644)
